...
class GraspNetROS:
    ...
    def process_grasp_results(self, grasp_results):
        ...
        try:
            ...
            # 获取物体标签（如果有的话）
            object_label = best_grasp.get('label', "未知物体")
            ...
            try:
                ...
                rospy.loginfo(f"发布最佳抓取位姿: " +
                              f"位置=({best_pose.pose.position.x:.3f}, {best_pose.pose.position.y:.3f}, {best_pose.pose.position.z:.3f}), " +
                              f"方向=(默认), 分数={best_grasp['score']:.3f}")
    ...
    def process_detected_objects(self, rgb, depth, poses_msg, header, camera_info):
        ...
        for i, pose in enumerate(poses_msg.poses):
            rospy.loginfo(f"为物体 {i+1} 生成抓取位姿，位置: ({pose.position.x:.3f}, {pose.position.y:.3f}, {pose.position.z:.3f})")
        ...
        # 处理所有收集到的抓取
        if all_grasps:
            rospy.loginfo(f"从所有物体中收集到 {len(all_grasps)} 个抓取候选")
            self.process_grasp_results(all_grasps)
        else:
            rospy.logwarn("没有为任何物体生成有效的抓取")
            self.publish_default_grasp()
