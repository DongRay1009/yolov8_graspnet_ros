#!/usr/bin/env python3
# filepath: d:\GitHub repo\yolov8_graspnet_ros\yolo_graspnet_ros\scripts\graspnet_generator.py
    ...
    def process_messages(self, rgb_msg, depth_msg, poses_msg=None):
        ...
        try:
            ...
            if poses_msg is None:
                rospy.loginfo("No detection results, will use entire scene for grasp pose generation")
                pose_count = 0
            else:
            ...
            if poses_msg is None or pose_count == 0:
                ...
                for i, pose in enumerate(poses_msg.poses):
                    ...
                    rospy.loginfo(f"Generating grasp poses for object {i+1}, position: ({position.x:.3f}, {position.y:.3f}, {position.z:.3f})")
